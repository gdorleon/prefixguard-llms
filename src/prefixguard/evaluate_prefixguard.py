...
# Load model and tokenizer, handle LLaMA special case
def load_model_and_tokenizer(model_name_or_path: str, num_labels: int):
    ...
    if is_llama:
        base_model = LlamaForSequenceClassification.from_pretrained(model_name_or_path, config=config)
    else:
        base_model = AutoModelForSequenceClassification.from_pretrained(model_name_or_path, config=config)

    return base_model, tokenizer, is_llama

# Run model forward pass over dataset and collect logits + labels
@torch.no_grad()
...
def main(args):
    # If training saved a prefixguard_config.json, reload key info from it
    cfg_path = os.path.join(args.model_name_or_path, "prefixguard_config.json")
    if os.path.exists(cfg_path):
        with open(cfg_path) as f:
            cfg = json.load(f)
        base_model_name = cfg.get("model_name_or_path", args.model_name_or_path)
        num_labels = cfg.get("num_labels", args.num_labels)
        max_seq_len = cfg.get("max_seq_length", args.max_seq_length)
    else:
    ...
    base_model, tokenizer, is_llama = load_model_and_tokenizer(
        base_model_name if not os.path.isdir(args.model_name_or_path) else args.model_name_or_path,
        num_labels=num_labels
    )

    # If PEFT adapters are saved in the model directory, attach them
    ...
    report = {"test": classification_report(labels, preds)}

    # Fairness by subgroup if `group` is available
    ...
    # Robustness check with obfuscated inputs
    if args.evaluate_obfuscation:
        ...
        report["robustness"] = {
            "macro_f1_drop": float(report["test"]["macro_f1"] - rep_obf["macro_f1"]),
            "accuracy_drop": float(report["test"]["accuracy"] - rep_obf["accuracy"]),
            "obfuscated": rep_obf
        }

    # Calibration with optional temperature scaling
